...
## trade_collection_coverage

지역별 계약월 수집 완료 상태를 저장한다.

- `region_id`: 수집 대상 시·군·구
- `contract_month`: 해당 월의 첫날
- `completed_at`: 모든 페이지 저장을 정상 완료한 시각
- 기본키: `(region_id, contract_month)`

거래가 0건인 지역도 API 수집이 정상 종료되면 완료로 기록한다. 특정 월의 완료 지역 수가 전체 2단계 지역 수와 같을 때만 완결된 수도권 월로 사용한다.

2026-07-22에 구버전 장기 수집 로그에서 성공한 8,522개 지역·월을 완료 체크포인트로 복구했다. 재개 수집기는 이 기록을 건너뛰고 지역코드 41450의 2021-03부터 신규 수집을 이어갔다.
...
- 계약 해제 거래는 제외한다.
- 같은 연도의 수도권 지역 평균 평단가 중간값을 구하고 각 지역의 `평단가 ÷ 중간값 × 100` 가격지수 구간으로 1~10급지를 부여한다.
